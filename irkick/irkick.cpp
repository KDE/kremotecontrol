...
void
IRKick::slotReloadConfiguration()
{
          ...
          if (!currentModeIcons[i.key()])
            {
              ...
              currentModeIcons[i.key()]->actionCollection()->action("file_quit")->setEnabled(
                  false);
            }
...
bool
IRKick::getPrograms(const IRAction &action, QStringList &programs)
{
...
void
IRKick::executeAction(const IRAction &action)
{
...
void
IRKick::gotMessage(const QString &theRemote, const QString &theButton,
...
void
IRKick::stealNextPress(QString app, QString module, QString method)
{
  npApp = app;
  npModule = module;
  npMethod = method;
